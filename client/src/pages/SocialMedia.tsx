...
import { Instagram, Clock, Target, TrendingUp, TrendingDown, AlertCircle, AlertTriangle } from "lucide-react";
...
export default function SocialMedia() {
  ...
  const category: ActivityCategory = "social_media";
  ...
  return (
    <div className="py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h2 className="text-2xl font-bold">Social Media Tracking</h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Monitor and manage your social media usage
          </p>
        </div>
        ...
        <Card className="shadow">
          <CardContent className="p-6">
            <div className="flex items-center">
              <div className="rounded-full p-3 bg-red-100 dark:bg-red-900/20 text-red-500 dark:text-red-300 mr-4">
                <Instagram size={24} />
              </div>
              <div>
                <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Social Media Time</div>
                {activitiesLoading ? (
      ...
      {(goalsLoading || !goalProgress) ? (
        ...
        <Card className="shadow mb-8">
          <CardContent className="p-6">
            <h3 className="text-lg font-medium mb-4">Social Media Goal</h3>
            ...
            {goalExceeded && (
              <Alert variant="destructive" className="mt-4">
                ...
                <AlertDescription>
                  You've exceeded your social media time goal. Consider taking a break.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <Card className="shadow">
          ...
          <CardContent>
            ...
            {/* Social media list */}
            <div className="mt-6">
              <h4 className="text-sm font-medium mb-2">Track by Platform</h4>
              <div className="space-y-2">
                {[
                  { name: "Instagram", color: "bg-pink-100 dark:bg-pink-900/20 text-pink-500 dark:text-pink-400", icon: <Instagram size={16} /> },
                  { name: "Facebook", color: "bg-blue-100 dark:bg-blue-900/20 text-blue-500 dark:text-blue-400", icon: <span className="text-sm">f</span> },
                  { name: "Twitter/X", color: "bg-gray-100 dark:bg-gray-900/20 text-gray-500 dark:text-gray-400", icon: <span className="text-sm">X</span> },
                  { name: "TikTok", color: "bg-teal-100 dark:bg-teal-900/20 text-teal-500 dark:text-teal-400", icon: <span className="text-sm">T</span> },
                  { name: "YouTube", color: "bg-red-100 dark:bg-red-900/20 text-red-500 dark:text-red-400", icon: <span className="text-sm">Y</span> },
                ].map(platform => (
                  <Button
                    key={platform.name}
                    variant="outline"
                    ...
                    onClick={() => {
                      // You could add platform-specific tracking here
                      if (!isRunning) {
                        start();
                      }
                    }}
                  >
                    <span className={`flex items-center justify-center w-6 h-6 rounded-full mr-2 ${platform.color}`}>
                      {platform.icon}
                    </span>
                    {platform.name}
                  </Button>
        ...
        <CardContent>
          {activitiesLoading ? (
            ...
            <div className="text-center py-4 text-gray-500 dark:text-gray-400">
              No social media sessions recorded yet
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="shadow mb-8">
        <CardHeader>
          <CardTitle>Tips for Healthy Social Media Usage</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex space-x-3">
              <AlertTriangle className="text-amber-500 dark:text-amber-400 flex-shrink-0" />
              <p className="text-sm">Set time limits for your social media apps</p>
            </div>
            <div className="flex space-x-3">
              <AlertTriangle className="text-amber-500 dark:text-amber-400 flex-shrink-0" />
              <p className="text-sm">Turn off notifications to reduce distractions</p>
            </div>
            <div className="flex space-x-3">
              <AlertTriangle className="text-amber-500 dark:text-amber-400 flex-shrink-0" />
              <p className="text-sm">Take regular breaks from screens</p>
            </div>
            <div className="flex space-x-3">
              <AlertTriangle className="text-amber-500 dark:text-amber-400 flex-shrink-0" />
              <p className="text-sm">Use "Focus" or "Do Not Disturb" modes</p>
            </div>
          </div>
        </CardContent>
      </Card>
